...
function ripple(event) {
    ...
    const rippleSize = Math.max(
        targetElmnt.clientWidth,
        targetElmnt.clientHeight
    );
    ...
    rippleElmnt.style.left = `${Math.round(
        event.clientX - targetBoundary.left - rippleSize / 2
    )}px`;
    rippleElmnt.style.top = `${Math.round(
        event.clientY - targetBoundary.top - rippleSize / 2
    )}px`;
