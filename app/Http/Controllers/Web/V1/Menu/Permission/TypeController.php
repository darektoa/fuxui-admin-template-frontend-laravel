...
use App\Http\Requests\V1\Menu\Permission\Type\{
    DestroyRequest,
    EditRequest,
    IndexRequest,
    ShowRequest,
...
class TypeController extends Controller
{
    ...
    public function index(IndexRequest $request)
    {
        try {
            ...
            $permissionTypes = $response->object()->data;
    ...
    public function edit(EditRequest $request, string $id)
    {
        try {
            $resType = Http::withAuthToken()
                ->withUserAgent($request->userAgent())
                ->acceptJson()
                ->get(env('API_BASE_URL') . "/menus/permissions/types/$id");
            ...
            $permissionType = $resType->object()->data;
            $permissionTypes = $resTypes->object()->data;
    ...
    public function store(StoreRequest $request)
    {
        try {
            $response = Http::withAuthToken()
                ...
                ->post(env('API_BASE_URL') . '/menus/permissions/types');
    ...
    public function update(UpdateRequest $request, string $id)
    {
        try {
            $response = Http::withAuthToken()
                ...
                ->acceptJson()
                ->asMultipart()
                ->post(env('API_BASE_URL') . "/menus/permissions/types/$id", array_merge($request->toArray(), [
                    '_method'   => 'PUT',
                ]));
            ...
            return back()
                ->with('success', 'Successfully updated permission type');
        } catch (Exception $e) {
            return redirect()->route('sign-in.index');
        }
    }
    ...
    public function destroy(DestroyRequest $request, string $id)
    {
        try {
            $response = Http::withAuthToken()
                ->withUserAgent($request->userAgent())
                ->acceptJson()
                ->delete(env('API_BASE_URL') . "/menus/permissions/types/$id");

            $response->throwIfClientError();
            $response->throwIfServerError();

            return back()
                ->with('success', 'Successfully deleted permission type');
